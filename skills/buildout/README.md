...
**v0.2 adds a buildout engine:** start from a verified-MIT boilerplate, assemble the UI coherently from the live shadcn registry pool (filtered + MIT-allowlisted), and store what you build for reuse via the companion `component-library` skill.
...
|-- data/
|   |-- allowlist.json          # MIT-verified registries (license + endpoint evidence, dated)
...
|   `-- items/                  # component catalogs: shadcn core 12 (hand-curated 2026-09-17) + velora / reui / cult-ui (generated) + overrides (curated)
|-- scripts/
|   |-- registry_sync.py        # sync | check | list | onboard | verify  (stdlib Python)
...
- Pool discipline: query `data/registries.snapshot.json` via `scripts/registry_sync.py list --match ...`; never paste raw registry JSON; refresh with `check` -> `sync`; run `verify` after any allowlist edit (404 templates and all-gated registries fail).
