...
| Q0b | for whom | "Is this for you, or for a client?" (client: their settings and keys stay in this project only) | `me` |
...
5. Read back a 5-line summary, then `dh phase done define` and continue (no gate here). Every choice the owner
   made in chat that the brief does not hold → `dh note decision "…"` now.
