...
**Exercised on a real template (not just fixtures):** `salmanshahriar/Next-Elite` (MIT, ★114, Next 16 + Better Auth + next-intl/RTL + Docker) was cloned through the factory to disk D — `npm install` green, `next build` green (15 routes), served on the template's own port with the on-disk BUILD_ID present in the page (freshness proved), and both gates behaved correctly on the live artifact: `swap_check` red on the unswapped clone (3 vendors, 38 references, exit 1) and the leak check red on the live page (31 hits of the template name, its author and its demo domain — the defect the rebrand phase exists to kill). Three real defects were found by that run and fixed in the pack: package-manager shims not resolving on Windows, installs running in the caller's cwd instead of the project, and a swap map written in the wrong shape.

Skill versions: buildout 0.8.0 · vps-ops 0.9.9 · deckhand-profile 0.3.3 · component-library 0.1.3. **78 stdlib tests green.**
